...
public class NewsSourceServiceImpl implements NewsSourceService {
	 ...
	 */
	@Autowired
	NewsSourceRepository newsSourceRepo;
	...
	public boolean addNewsSource(NewsSource newsSource) {
		NewsSource added = newsSourceRepo.insert(newsSource);
		if (added != null) {
			return true;
		} else {
			return false;
	...
	public boolean deleteNewsSource(int newsSourceId) {
		try {
			Optional<NewsSource> search = newsSourceRepo.findById(newsSourceId);
			if (search.isPresent()) {
	...
	public NewsSource updateNewsSource(NewsSource newsSource, int newsSourceId) throws NewsSourceNotFoundException {
		try {
			Optional<NewsSource> oldSource = newsSourceRepo.findById(newsSourceId);
	...
	public List<NewsSource> getAllNewsSourceByUserId(String createdBy) {
		List<NewsSource> newsSourceList = newsSourceRepo.findAllNewsSourceByNewsSourceCreatedBy(createdBy);
