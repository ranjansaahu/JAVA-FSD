...
public class SwaggerConfig {
	...
	public Docket productApi() {
    	return new Docket(DocumentationType.SWAGGER_2).select()
				.apis(RequestHandlerSelectors.basePackage("com.stackroute.userprofile")).build().apiInfo(getUserProfileInfo())
				.useDefaultResponseMessages(false);

	}

	private ApiInfo getUserProfileInfo() {
		ApiInfoBuilder apibuilder = new ApiInfoBuilder();
		apibuilder.title("UserProfile Assignment").version("V1.0")
		          .description("UserProfile Details Can Be Registered and Verified")
