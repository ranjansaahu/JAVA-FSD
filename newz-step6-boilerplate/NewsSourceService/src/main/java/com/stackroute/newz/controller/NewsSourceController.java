...
public class NewsSourceController {
	...
	public ResponseEntity<?> addNewsSource(@RequestBody() NewsSource newsSource) {
		try {
			boolean addedSource = newsSourceService.addNewsSource(newsSource);
			if (addedSource) {
				return new ResponseEntity<String>("NewsSource Created", HttpStatus.CREATED);
			} else {
				return new ResponseEntity<String>("NewSource Not Created", HttpStatus.CONFLICT);
			}
		} catch (Exception e) {
	...
	public ResponseEntity<?> deleteNewsSource(@PathVariable("newssourceId") int newssourceId) {
		try {
			boolean result = newsSourceService.deleteNewsSource(newssourceId);
			if (result) {
				return new ResponseEntity<String>("NewsSource Deleted", HttpStatus.OK);
			} else {
				return new ResponseEntity<String>("NewsSource Not Deleted", HttpStatus.NOT_FOUND);
			}
		} catch (Exception e) {
			return new ResponseEntity<String>(e.getMessage(), HttpStatus.NOT_FOUND);
		}
	...
	public ResponseEntity<?> updateNewsSource(@PathVariable("newssourceId") int newssourceId,
			@RequestBody() NewsSource newsSource) {
		try {
			NewsSource updatedNewsSource = newsSourceService.updateNewsSource(newsSource, newssourceId);
			return new ResponseEntity<NewsSource>(updatedNewsSource, HttpStatus.OK);
	...
	public ResponseEntity<?> getSpecificNewsSource(@PathVariable("newssourceId") int newssourceId,
			@PathVariable("userId") String userId) {
		try {
			NewsSource searchNewsSource = newsSourceService.getNewsSourceById(userId, newssourceId);
			return new ResponseEntity<NewsSource>(searchNewsSource, HttpStatus.OK);
		} catch (NewsSourceNotFoundException e) {
			return new ResponseEntity<String>(e.getMessage(), HttpStatus.NOT_FOUND);
		}
	}
	...
	public ResponseEntity<?> getAllNewsSourceByUser(@PathVariable("userId") String userId) {
		try {
			List<NewsSource> newsSourceList = newsSourceService.getAllNewsSourceByUserId(userId);
			return new ResponseEntity<List<NewsSource>>(newsSourceList, HttpStatus.OK);
		} catch (Exception e) {
			return new ResponseEntity<String>(e.getMessage(), HttpStatus.NOT_FOUND);
		}
	}
