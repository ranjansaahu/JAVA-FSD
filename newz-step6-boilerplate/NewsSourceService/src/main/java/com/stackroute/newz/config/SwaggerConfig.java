...
public class SwaggerConfig {
	...
	public Docket productApi() {
		return new Docket(DocumentationType.SWAGGER_2).select()
				.apis(RequestHandlerSelectors.basePackage("com.stackroute.newz")).build().apiInfo(getNewsSourceInfo())
				.useDefaultResponseMessages(false);

	}

	private ApiInfo getNewsSourceInfo() {
		ApiInfoBuilder apibuilder = new ApiInfoBuilder();
		apibuilder.title("NewsSource Assignment").version("V1.0")
		          .description("NewsSource Details Can Be Registered and Verified")
